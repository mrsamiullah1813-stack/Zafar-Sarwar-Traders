...
function mapDbOrderToCustomerOrder(r: any): CustomerOrder {
  ...
  return {
    ...
    payment_proof_url: r.payment_proof_url || r.paymentProofUrl || undefined,
    paymentProofFileName: r.payment_proof_file_name || r.paymentProofFileName || undefined,
    payment_proof_file_name: r.payment_proof_file_name || r.paymentProofFileName || undefined,
    paymentProofUploadedAt: r.payment_proof_uploaded_at || r.paymentProofUploadedAt || undefined,
    payment_proof_uploaded_at: r.payment_proof_uploaded_at || r.paymentProofUploadedAt || undefined,
...
export async function createOrderInSupabase(order: CustomerOrder): Promise<{ success: boolean; error?: string }> {
  ...
  const orderPayload: Record<string, any> = {
    ...
    payment_method: order.paymentMethodName || order.paymentMethodId || ((order.paymentProofUrl || (order as any).payment_proof_url) ? 'Online Transfer' : 'Cash on Delivery'),
    paymentStatus: order.paymentStatus,
    payment_status: order.paymentStatus || ((order.paymentProofUrl || (order as any).payment_proof_url) ? 'Payment Proof Submitted' : 'Cash on Delivery'),
    paymentProofUrl: order.paymentProofUrl || (order as any).payment_proof_url || null,
    payment_proof_url: order.paymentProofUrl || (order as any).payment_proof_url || null,
    paymentProofFileName: order.paymentProofFileName || (order as any).payment_proof_file_name || null,
    payment_proof_file_name: order.paymentProofFileName || (order as any).payment_proof_file_name || null,
    paymentProofUploadedAt: order.paymentProofUploadedAt || (order as any).payment_proof_uploaded_at || null,
    payment_proof_uploaded_at: order.paymentProofUploadedAt || (order as any).payment_proof_uploaded_at || null,
